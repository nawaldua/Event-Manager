...
public class EventManager {
	...
	public boolean removeEvent(String name) {
		...
		for (Event e: eventList) {
			...
			if (e.getName().equalsIgnoreCase(name)){
				// get the event's volunteers
				if (e.getAdjacentNodes() == null) {
					// remove the event
					removeEvent(name);
					return true;
				}
				else {
					List<GraphNode> eventsvolunteers = e.getAdjacentNodes();
					// for each volunteer
					for (GraphNode v: eventsvolunteers) {
						// remove the event from the volunteer's events
	...
	public boolean removeVolunteer(String name){
		...
		for (Volunteer v: volunteerList) {
			// if the volunteer is found
			if (v.getName().equalsIgnoreCase(name)){
				// get the volunteer's events
				if (v.getAdjacentNodes() == null) {
					// remove the volunteer
					removeVolunteer(name);
					return true;
				}
				else {
					List<GraphNode> volunteersevents = v.getAdjacentNodes();
					// for each event
					for (GraphNode e: volunteersevents) {
						// remove the volunteer from the event's volunteers
	...
	public void displayAllEvents(){
		...
		if (numEvents == 0) {
			System.out.format(Resource.STR_ERROR_DISPLAY_EVENT_FAILED);
		}
		else {
			System.out.format(Resource.STR_DISPLAY_ALL_EVENTS_PRINT_FORMAT, numEvents);
			for (Event e: eventList) {
				e.toString();
			}
	...
	public void displayAllVolunteers(){
		int numVolunteers = volunteerList.size();
		if (numVolunteers == 0) {
			System.out.format(Resource.STR_ERROR_DISPLAY_VOLUNTEER_FAILED);
		}
		else {
			System.out.format(Resource.STR_DISPLAY_ALL_VOLUNTEERS_PRINT_FORMAT, numVolunteers);
			for (Volunteer v: volunteerList) {
				v.toString();
			}
